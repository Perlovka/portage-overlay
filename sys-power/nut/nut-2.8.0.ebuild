...
IUSE="cgi gui ipmi snmp +usb modbus selinux split-usr ssl systemd tcpd xml zeroconf"
...
for name in ${MODBUS_DRIVERLIST} ; do
	IUSE_UPS_DRIVERS="${IUSE_UPS_DRIVERS} ups_drivers_${name}"
	REQUIRED_USE="${REQUIRED_USE} ups_drivers_${name}? ( modbus )"
done
...
PATCHES=(
	...
	"${FILESDIR}"/nut-2.8.0-fix-icon-lookup.patch
)
...
src_install() {
	...
	if use gui; then
		...
		doins -r scripts/python/app/pixmaps
		doins -r scripts/python/app/ui

		sed -i -e 's/;HardwareSettings;Settings;Qt//' -e 's/NUT-Monitor-py3qt5/nut-monitor/' scripts/python/app/${PN}-monitor-py3qt5.desktop || die

		local size
		for size in 48 64 256; do
			doicon -s ${size} scripts/python/app/icons/${size}x${size}/${PN}-monitor.png
		done
		doicon -s scalable scripts/python/app/icons/scalable/${PN}-monitor.svg

		newmenu scripts/python/app/${PN}-monitor-py3qt5.desktop nut-monitor.desktop
	fi
