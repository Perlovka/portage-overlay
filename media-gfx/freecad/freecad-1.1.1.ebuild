...
if [[ ${PV} == *9999* ]]; then
	...
	SRC_URI="
		https://github.com/${MY_PN}/${MY_PN}/releases/download/${PV}/freecad_source_${PV}.tar.gz -> ${P}.tar.gz
	"
	KEYWORDS="~amd64"
fi
...
PATCHES=(
	"${FILESDIR}"/${PN}-1.1.0-tests-src-Qt-only-build-test-for-BUILD_GUI-ON.patch
...
src_prepare() {
	...
	sed -e 's#@CMAKE_INSTALL_BINDIR@#/usr/bin#g' -i src/XDGData/FreeCAD.thumbnailer.in  || die
