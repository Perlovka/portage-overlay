...
SRC_URI="https://github.com/libretro/${PN}/archive/9022565efeb1a49584542b923cfcdaa90431d243.tar.gz -> ${P}.tar.gz"
#SRC_URI="https://github.com/libretro/${PN}/archive/v${PV}.tar.gz -> ${P}.tar.gz"
RESTRICT="primaryuri"

S="${WORKDIR}/RetroArch-9022565efeb1a49584542b923cfcdaa90431d243"
...
IUSE="+7zip alsa +armvfp +assets +cheevos +database dbus debug discord
	dispmanx egl ffmpeg gles2 gles3 jack +joypad_autoconfig kms libass libcaca
	libsixel libusb materialui mbedtls miniupnpc mpv +neon +network openal +opengl
	osmesa oss overlay pulseaudio qt sdl sdl2 shaders +truetype +threads +udev v4l2
...
RDEPEND="
	games-emulation/libretro-core-info:0=
	...
	shaders? ( games-emulation/libretro-slang-shaders )
...
src_prepare() {
	...
	sed -i retroarch.cfg \
		-e 's:# \(libretro_info_path =\):\1 "/usr/share/libretro/info/":' \
		-e 's:# \(assets_directory =\):\1 "/usr/share/retroarch/assets/":' \
		-e 's:# \(joypad_autoconfig_dir =\):\1 "/usr/share/retroarch/autoconfig/":' \
		-e 's:# \(cheat_database_path =\):\1 "/usr/share/libretro/database/cht/":' \
		-e 's:# \(content_database_path =\):\1 "/usr/share/libretro/database/rdb/":' \
		-e 's:# \(overlay_directory =\):\1 "/usr/share/libretro/overlays/":' \
		-e 's:# \(video_shader_dir =\):\1 "/usr/share/libretro/shaders/shaders_slang/":' \
		...
		-e 's:# \(libretro_directory =\):\1 "~/.local/share/retroarch/cores/":' \
