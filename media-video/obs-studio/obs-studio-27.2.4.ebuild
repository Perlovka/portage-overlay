...
OBS_VST_COMMIT="8ad3f64e702ac4f1799b209a511620eb1d096a01"
...
if [[ ${PV} == 9999 ]]; then
	...
	SRC_URI+=" vst? ( https://github.com/obsproject/obs-vst/archive/${OBS_VST_COMMIT}.zip -> obs-vst-${OBS_VST_COMMIT}.zip )"
...
IUSE="
	...
	pulseaudio python speex +ssl truetype v4l vlc vst wayland
"
REQUIRED_USE="
...
src_unpack() {
	...
	if [[ ${PV} == 9999 ]]; then
		...
		if use vst; then
			rm -d ${P}/plugins/obs-vst || die
			mv obs-vst-${OBS_VST_COMMIT} ${P}/plugins/obs-vst || die
		fi
	fi
}
