...
RDEPEND="${DEPEND}
	...
	>=dev-embedded/arduino-builder-1.4.8
...
PATCHES=(
	...
	"${FILESDIR}/${P}-xdg-compliance.patch"
)
...
src_prepare() {
	...
	# Unbundle avr toolchain and libastyle
	sed -i -e 's/target name="avr-toolchain-bundle" unless="light_bundle"/target name="avr-toolchain-bundle" if="never"/' \
		-e 's/\(target name="linux-libastyle-[a-zA-Z0-9]*"\)/\1 if="never"/g' "build/build.xml" || die
	...
	# Fix arduino-builder path
	sed -i 's@BaseNoGui.getContentFile("arduino-builder").getAbsolutePath()@"/usr/bin/arduino-builder"@' "${S}/arduino-core/src/cc/arduino/Compiler.java" || die
}
...
src_install() {
	...
	for icon_dir in lib/icons/*; do
		newicon -s ${icon_dir##*x} "${icon_dir}/apps/arduino.png" "${PN}.png"
	done
}
